...
-- 1-	What is the total sales amount for each category?
 ...
 select Category, sum(sale_amount) AS Total_Sales_Amount from sales
 group by Category;

-- 2-	Which categories have more than 10 sales?

select Category, count(sale_id) AS Count_Sales from sales
group by Category
having count(sale_id) > 10 ;

-- 3-	What is the maximum sale amount in each category?

select Category, max(sale_amount) AS MAX_SALE_AMOUNT from sales
group by Category;
...
select Category, avg(sale_amount) AS Avg_Sales from sales
group by Category
having avg(sale_amount) < 50 ;

-- 5-	How many sales does each product have?

select product_name, count(sale_id) AS Total_Sale from sales
...
select product_name, avg(sale_amount) AS AVG_SaleAmount from sales
group by product_name
having count(*) >2 ;

-- 8-	Which products have the highest sale amount above 100?

select product_name, max(sale_amount) AS MAX_Sale from sales
group by product_name
...
select product_name, max(sale_amount) AS Highest_Sale from sales
group by product_name;

-- 10-	Which categories have a total sales amount above 800?

select category, sum(sale_amount) AS Total_Sale from sales
group by category;